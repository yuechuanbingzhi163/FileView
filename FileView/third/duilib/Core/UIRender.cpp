...
//add by hdw
std::wstring A2W(const char *lpStr)
{
	int unicodeLen = ::MultiByteToWideChar(CP_ACP, 0, lpStr, -1, NULL, 0);

	wchar_t *pUnicode;
	pUnicode = new wchar_t[unicodeLen + 1];
	ZeroMemory(pUnicode, sizeof(wchar_t) * (unicodeLen + 1));

	::MultiByteToWideChar(CP_ACP, 0, lpStr, -1, (LPWSTR)pUnicode, unicodeLen);
...
std::string W2A(const wchar_t *lpStr)
{
	int charLen = ::WideCharToMultiByte(CP_ACP, 0, lpStr, -1, NULL, 0, NULL, NULL);

	char *pChar;
	pChar = new char[charLen + 1];
	ZeroMemory(pChar, sizeof(char) * (charLen + 1));

	::WideCharToMultiByte(CP_ACP, 0, lpStr, -1, pChar, charLen, NULL, NULL);
...
#else
			graphics.MeasureString(A2W(pstrText).c_str(), -1, &font, rectF, &stringFormat, &bounds);
#endif
...
#else
			graphics.DrawString(A2W(pstrText).c_str(), -1, &font, rectF, &stringFormat, &brush);
#endif			
		}
