...
//		if( m_bSelected == bSelected ) return;
	...
	void COptionUI::PaintStatusImage(HDC hDC)
	{
		...
		else if( (m_uButtonState & UISTATE_SELECTED) != 0 && IsEnabled()) 
        {
        ...
        {
            if( !m_sSelectedDisableImage.IsEmpty() ) {
                if( !DrawImage(hDC, (LPCTSTR)m_sSelectedDisableImage) ) m_sSelectedDisableImage.Empty();
                else goto Label_ForeImage;
            }
