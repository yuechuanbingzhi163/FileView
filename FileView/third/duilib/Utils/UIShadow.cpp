    ...
    case WM_PRINTCLIENT:
        {
            ...
            //pThis->PrintClient((HDC)wParam, hwnd);
            break;
        }
	...
	case WM_SYSCOMMAND:
		{
			if (pThis->m_Status & SS_ENABLED)
			{
				if (SC_MAXIMIZE == wParam || SC_MINIMIZE == wParam)
				{
					::ShowWindow(pThis->m_hWnd, SW_HIDE);
					pThis->m_Status &= ~(SS_VISABLE);
				}				
...
void CShadowUI::PrintClient( HDC hdcDest, HWND hParent )
{
    RECT WndRect;
    GetWindowRect(hParent, &WndRect);
    int nShadWndWid;
    ...
    BOOL bRet = ::BitBlt(hdcDest, ptDst.x, ptDst.y, nShadWndWid, nShadWndHei, hMemDC, 0, 0, SRCCOPY);
